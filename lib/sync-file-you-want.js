  ...
  toggle: function() {
    ...
    if(this.modalPanel.isVisible()) {
      ...
      if(editor === null || editor === undefined) {
        this.syncFileYouWantView.setErrorContent("Please run the sync file command in a file view");
        this.displayMessage();
        return;
      }
      ...
      for(i = 0; i < mappings.length; i++) {
        ...
        console.log('sourceLoc is: ' + sourceLoc);
        console.log('destLoc is: ' + destLoc);
      ...
      if(!found) {
        this.syncFileYouWantView.setErrorContent("Your current file location is not in one of your source locations, or the relevent dest Location is empty. Please set the settings file properly and retry.");
        this.displayMessage();
        return;
      }
      console.log('destFileName is: ' + destFileName);
      fs.createReadStream(fileName).pipe(fs.createWriteStream(destFileName));
      this.syncFileYouWantView.setErrorContent("Your file is successfully copied to destination location!");
      this.displayMessage();
    }
  },
